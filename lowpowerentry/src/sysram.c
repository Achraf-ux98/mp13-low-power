...
#define TAMP_BASE_ADDR     0x5C00A000U
...
#define TAMP_BKP2R_OFFSET      0x108U
...
#define SYSRAM_DDR_SW_SELF_REFRESH_MODE 0x0U
#define SYSRAM_DDR_AUTO_SELF_REFRESH_MODE 0x1U
#define SYSRAM_DDR_HW_SELF_REFRESH_MODE 0x2U
#define SYSRAM_DDR_INVALID_MODE 0x3U
...
static uint32_t ddr_sr_read_mode(void)
{
	const uint32_t pwrctl_addr = DDRCTRL_BASE_ADDR + DDRCTRL_PWRCTL_OFFSET;
	...
	switch (pwrctl & (DDRCTRL_PWRCTL_EN_DFI_DRAM_CLK_DISABLE |
				 DDRCTRL_PWRCTL_SELFREF_EN)) {
	case 0U:
		return SYSRAM_DDR_SW_SELF_REFRESH_MODE;
	case DDRCTRL_PWRCTL_EN_DFI_DRAM_CLK_DISABLE:
		return SYSRAM_DDR_HW_SELF_REFRESH_MODE;
	case DDRCTRL_PWRCTL_EN_DFI_DRAM_CLK_DISABLE | DDRCTRL_PWRCTL_SELFREF_EN:
		return SYSRAM_DDR_AUTO_SELF_REFRESH_MODE;
	default:
		return SYSRAM_DDR_INVALID_MODE;
	}
}
...
static HAL_StatusTypeDef sysram_ddr_sr_setmode(uint32_t mode)
{
	HAL_StatusTypeDef ret;

	switch (mode) {
	case SYSRAM_DDR_SW_SELF_REFRESH_MODE:
		ret = ddr_sr_mode_ssr();
		break;
	case SYSRAM_DDR_AUTO_SELF_REFRESH_MODE:
		ret = ddr_sr_mode_asr();
		break;
	case SYSRAM_DDR_HW_SELF_REFRESH_MODE:
		ret = ddr_sr_mode_hsr();
		break;
	default:
		ret = HAL_ERROR;
...
static uint32_t sysram_ddr_sr_readmode(void)
{
	return ddr_sr_read_mode();
}

/* RTC handoff helper: fetch FSBL entry from BKP_DR2 and jump to it. */
__attribute__((noreturn))
static void sysram_jump_to_fsbl_from_bkp_dr2(void)
{
	uintptr_t jump_target;
	const uint32_t bkp2r_addr = TAMP_BASE_ADDR + TAMP_BKP2R_OFFSET;

	/* Direct read avoids HAL RTC call dependencies during SYSRAM handoff. */
	jump_target = (uintptr_t)mmio_read32(bkp2r_addr);
	__asm__ volatile("bx %0" : : "r"(jump_target) : "memory");
...
static void ddr_sr_stub(struct ddr_sr_result *result)
{
	...
	if (sysram_ddr_sr_setmode(SYSRAM_DDR_SW_SELF_REFRESH_MODE) != HAL_OK) {
		result->phase = 0xE0U;
		return;
	}

	if (sysram_ddr_sr_readmode() != SYSRAM_DDR_SW_SELF_REFRESH_MODE) {
		result->phase = 0xE4U;
		return;
	}
	...
	sysram_jump_to_fsbl_from_bkp_dr2();
