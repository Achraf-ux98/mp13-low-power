 ...
 *(.rela.*)
    ...
    __rom_region_start = (0xc0000000 + 0x0);
    rom_start :
    {
...
.intList :
{
 ...
 .debug_sup 0 : { *(.debug_sup) }
    ...
    SYSRAM (NOLOAD) : { __SYSRAM_start = .; KEEP(*(SYSRAM)) KEEP(*(SYSRAM.*)) __SYSRAM_end = .; } > SYSRAM __SYSRAM_size = __SYSRAM_end - __SYSRAM_start; __SYSRAM_load_start = LOADADDR(SYSRAM); DDR_CODE (NOLOAD) : { __DDR_CODE_start = .; KEEP(*(DDR_CODE)) KEEP(*(DDR_CODE.*)) __DDR_CODE_end = .; } > DDR_CODE __DDR_CODE_size = __DDR_CODE_end - __DDR_CODE_start; __DDR_CODE_load_start = LOADADDR(DDR_CODE); DDR_DATA (NOLOAD) : { __DDR_DATA_start = .; KEEP(*(DDR_DATA)) KEEP(*(DDR_DATA.*)) __DDR_DATA_end = .; } > DDR_DATA __DDR_DATA_size = __DDR_DATA_end - __DDR_DATA_start; __DDR_DATA_load_start = LOADADDR(DDR_DATA); ETH_SRAM (NOLOAD) : { __ETH_SRAM_start = .; KEEP(*(ETH_SRAM)) KEEP(*(ETH_SRAM.*)) __ETH_SRAM_end = .; } > ETH_SRAM __ETH_SRAM_size = __ETH_SRAM_end - __ETH_SRAM_start; __ETH_SRAM_load_start = LOADADDR(ETH_SRAM);
    .last_section :
    {
      KEEP(*(.last_section))
    } > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = (3489660928) + (268435456);
