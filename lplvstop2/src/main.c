...
__attribute__((section("SYSRAM"), aligned(8)))
static uint8_t fsbl_handoff_stack[1024];
...
int main(void)
{
	...
	printk("step: entering system clock config\n\r");
	SystemClock_Config();
	printk("step: system clock configured\n\r");
  
	HAL_RTC_Init(&RTCHandle_BKUP);
	printk("step: rtc initialized\n\r");
 
#if (USE_STPMIC1x) && !defined(USE_DDR)
	/* Configure PMIC */
	BSP_PMIC_Init();
	printk("step: pmic initialized\n\r");
	printk("step: before pmic regulators init\n\r");
	BSP_PMIC_InitRegulators();
	printk("step: pmic regulators initialized\n\r");
#endif
	...
	printk("step: gpio init skipped\n\r");
  
	printk("Hello World - Landed in user application in DDR\n\r");

	/* USER CODE BEGIN 2 */
	printk("step: user code begin 2\n\r");
...
// add your config test
	printk("step: user code begin 3\n\r");

	/* USER CODE END 3 */

	/* USER CODE BEGIN 4*/
	printk("Hello World - USE_STM32MP13XX_DK BOARD    \n\r");
	printk("step: user code begin 4\n\r");
	/* USER CODE END 4 */
...
{
#if !defined(USE_DDR)
	printk("step: sysclock start\n\r");
	HAL_RCC_DeInit();
	printk("step: rcc deinit done\n\r");
	...
	printk("step: before osc config\n\r");
	if (HAL_RCC_OscConfig(&RCC_OscInitStructure) != HAL_OK) {
		/* HAL RCC configuration error */
		Error_Handler();
		}
	printk("step: after osc config\n\r");
	...
	printk("step: before clock config\n\r");
	if (HAL_RCC_ClockConfig(&RCC_ClkInitStructure) != HAL_OK) {
		/* HAL RCC configuration error */
		Error_Handler();
		}
	printk("step: after clock config\n\r");
...
static void PrepareFsblStackHandoff(void)
{
	uint32_t *fsbl_sp_slot = (uint32_t *)0x5c00a110U;
	uint32_t handoff_sp = (uint32_t)&fsbl_handoff_stack[sizeof(fsbl_handoff_stack) - 8U];

	*fsbl_sp_slot = handoff_sp;
	printk("step: fsbl sp slot=0x%08x value=0x%08x (sysram stack)\n\r", (uint32_t)fsbl_sp_slot, handoff_sp);
}
